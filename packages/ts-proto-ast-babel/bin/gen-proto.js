...
const PARENTPATH = resolve(join(__dirname, '..'));
const PROTOFOLDER = 'out/proto';
const OUTPROTOFOLDER = 'src/proto';
const OUTPATH = resolve(PARENTPATH, OUTPROTOFOLDER);
const PROTOPATH = resolve(join(PARENTPATH, PROTOFOLDER));
...
const main = async () => {
    ...
    paths.forEach(async tsFile => {
        ...
        if (currentPackage) {
            ...
            const file = join(OUTPATH, '.', fileName.replace(PROTOPATH, ''))
            mkdirp(dirname(file));
